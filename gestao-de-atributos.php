...
if (is_user_logged_in() && current_user_can('manage_attributes')) {        
        ...
        if (mysqli_num_rows($resultado_attribute) == 0) {    
            ...
            <table class="mytable">
                ...
                <?php
                    ...
                    $query_attributes = "SELECT attribute.name, attribute.obj_id, attribute.id, attribute.value_type, attribute.form_field_name, attribute.form_field_type, attribute.unit_type_id, attribute.form_field_order, attribute.form_field_size, attribute.mandatory, attribute.state
                    ...
                    $lines_attributes = mysqli_num_rows($result_attributes); 
                        ...
                        while ($array_attributes = mysqli_fetch_array($result_attributes)) {              
                            ...
                            <tr colspan="1" rowspan="<?php echo $lines_attributes?>">
                                <td><?php
                                 $query_obj = "SELECT DISTINCT name
                                 FROM object
                                 WHERE id='{$array_attributes['obj_id']}'";

                                $resultado_obj = executa_query($query_obj); // Passa $query_obj como parametro de executa_query e coloca o resultado na variavel $resultado_obj

                                $array_obj = mysqli_fetch_assoc($resultado_obj);
                                echo $array_obj['name'];?>
                                </td>
                                <td><?php
         ...
         <form name="gestao_de_atributos"  method="post">
            ...
            <p>
                <label><b>Obrigatorio:</b></label>
                    <input type="radio" name="obrigatorio" value="1"> <?php echo "Sim";?>
                    <input type="radio" name="obrigatorio" value="0"> <?php echo "Não";?>
            </p>
            <p>
    ...
    <?php
        ...
        $object_nome_formulario = guarda_variavel($_REQUEST['nome_formulario']);
        ...
        if (empty($object_nome_do_atributo)) {
            ...
            <?php
            back();   
        } elseif (empty($object_nome_formulario)) {
            ?>
